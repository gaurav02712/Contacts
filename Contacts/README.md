# SwiftUI Contact Card

Simple SwiftUI app to display a contact card with image, name, role, and links.  
Useful for layout learning and SwiftUI practice.

# Contacts App Take-Home Assignment
...
## Problem Statement
Design and implement a simple Contacts App using SwiftUI that provides basic functionality similar to the native iPhone Contacts app.
...
- **Delete a Contact:**
  ...
  - Add confirmation alert for deletion in edit screen (Bonus).
...
1. How would you persist the contact list between app sessions?  
   Answer: Use Core/Swift Data, file-storage with FileManager or SQLite.

2. Should we use a remote data base for this application?  
   Answer: Potentially yes, simply to serve as backup storage in case a user were to lose their device.
